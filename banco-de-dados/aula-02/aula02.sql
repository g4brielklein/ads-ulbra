create table tbgenero (
  ...
  nomegen varchar(20) not null unique
);
...
alter table tbfilme add constraint tbfilme_fkcodgen foreign key(fkcodgen)
  references tbgenero(pkcodgen) on delete set null;
alter table tbfilme add constraint tbfilme_fkcodclass foreign key(fkcodclass)
  references tbclassificacao(pkcodclass) on delete set null;

create table tbclassificacao (
  pkcodclass integer not null,
  nomeclass varchar(20) not null unique,
...
alter table tbrelfilmeloc add constraint tbrelfilmeloc_fkcodfilme foreign key(fkcodfilme) references tbfilme(pkcodfilme) on delete cascade;
alter table tbrelfilmeloc add constraint tbrelfilmeloc_fkcodloc foreign key(fkcodloc) references tblocacao(pkcodloc) on delete cascade;
...
alter table tblocacao add constraint tblocacao_fkcodcli foreign key(fkcodcli) references tbcliente(pkcodcli) on delete cascade;
